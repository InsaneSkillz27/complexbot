...
function initModule(options){
    ...
    writeFile("cssmodule" , "cssmodule" , options);
    writeFile("README.md" , "README.md" , options);
    ...
    writeFile("README.md" , "app/tpls/README.md" , options);
    writeFile("README.md" , "docs/README.md" , options);
...
function installDepModule(){
    if(shell.exec("fpm install --file app/index.html --module glue --module artTemplate").code !== 0){
        console.log("Error: fpm install failed");
    }
  /*
    if(shell.exec("fpm install --file app/index.html --module glue").code !== 0){
        console.log("Error: fpm install failed");
    }else{
      if(shell.exec("fpm install --file app/index.html --module artTemplate").code !== 0){
         console.log("Error: fpm install failed");
      }
    }
  */
...
function startModule(options){
     options.appname = options.appname || path.basename(process.cwd()).replace(/[^\w\s]+?/g, ' ');
     ...
     try{
         ...
         // installNpm();
         installDepModule();
         console.log('module create complete.');
     }catch(error){
...
myModule.line = function (argv) {
    var options = myModule.options(argv);
    return myModule(options);
};

myModule.options = function (argv) {
